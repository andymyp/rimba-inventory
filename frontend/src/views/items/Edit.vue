...
export default {
  setup() {
    ...
    function onFileSelected(event) {
      let fd = new FormData();
      fd.append("barang", event.target.files[0]);

      api.put(`/api/items/image/${route.params.id}`, fd).then((res) => {
        barang.gambar = res.data.data.barang;
      });
    }

    function update() {
      api.put(`/api/items/${route.params.id}`, item).then(() => {
        router.push({
          name: "items.index",
        });
      });
    ...
    return { item, barang, router, update, onFileSelected, apiUrl };
  },
};
</script>
